...
export async function updateAssignment(formData: FormData) {
  ...
  const supabase = await createClient(); const { error } = await supabase.rpc("update_owned_assignment_ready", { p_assignment_id: id, p_title: values.title, p_description: values.description, p_kind: values.kind, p_due_at: values.due_at, p_duration_minutes: values.duration_minutes, p_max_attempts: values.max_attempts, p_show_score_after_submit: values.show_score_after_submit, p_show_answers_after_submit: values.show_answers_after_submit, p_shuffle_questions: values.shuffle_questions, p_shuffle_options: values.shuffle_options, p_class_ids: values.class_ids, p_exam_mode: values.exam_mode, p_exam_require_fullscreen: values.exam_require_fullscreen, p_exam_track_focus_exits: values.exam_track_focus_exits, p_exam_allowed_focus_exits: values.exam_allowed_focus_exits, p_exam_violation_action: values.exam_violation_action, p_question_display_mode: values.question_display_mode, p_show_feedback_after_each_question: values.show_feedback_after_each_question });
...
export async function duplicateAssignment(formData: FormData) {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("duplicate_owned_assignment_ready", { p_assignment_id: id });
  if (error || !data) {
