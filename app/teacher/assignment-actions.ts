...
export async function releaseTestQuestions(formData: FormData) {
  ...
  if (error) { console.error(`release_owned_test_questions failed: code=${error.code}; message=${error.message}; details=${error.details ?? "none"}; hint=${error.hint ?? "none"}`); redirect(message(path, "error", "The assessment could not be opened.")); }
