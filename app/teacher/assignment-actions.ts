...
function settings(formData: FormData) {
  ...
  const exam_mode = checked(formData.get("exam_mode")); const exam_allowed_focus_exits = exam_mode ? integer(formData.get("exam_allowed_focus_exits"), 2) : 2; const exam_violation_action = "auto_submit"; const question_display_mode = text(formData.get("question_display_mode")) || "one_at_a_time";
  if (exam_allowed_focus_exits === null || exam_allowed_focus_exits < 0 || !["one_at_a_time", "all_at_once"].includes(question_display_mode)) return null;
  return { title, description, kind, due_at: due, duration_minutes: duration, max_attempts: maxAttempts, show_score_after_submit: checked(formData.get("show_score_after_submit")), show_answers_after_submit: checked(formData.get("show_answers_after_submit")), show_feedback_after_each_question: checked(formData.get("show_feedback_after_each_question")), question_display_mode, shuffle_questions: checked(formData.get("shuffle_questions")), class_ids: classIds(formData), exam_mode, exam_require_fullscreen: exam_mode ? checked(formData.get("exam_require_fullscreen")) : true, exam_track_focus_exits: exam_mode ? checked(formData.get("exam_track_focus_exits")) : true, exam_allowed_focus_exits, exam_violation_action };
}
...
export async function duplicateAssignment(formData: FormData) {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("duplicate_owned_assignment_with_presentation", { p_assignment_id: id });
  if (error || !data) {
