...
export default function StarClassroom({ initialState, currentWeekLabel, embedded = false, initialAssignments = [], initialGradeColumns = [], availableAssessments: initialAvailableAssessments = [], initialCwRecords = [] }: { initialState: ClassroomStarState; currentWeekLabel: string; embedded?: boolean; initialAssignments?: ClassroomHomeworkAssignment[]; initialGradeColumns?: ClassroomGradeColumn[]; availableAssessments?: AvailableClassroomAssessment[]; initialCwRecords?: ClassroomCwRecord[] }) {
  ...
  function sheetColumnHeader(sheetColumn: SheetDatedColumn) {
    ...
    if (sheetColumn.type === "work") {
      ...
      return <th className={`${gradebookStyles.datedColumnHead} ${item.kind === "homework" ? gradebookStyles.homeworkHead : gradebookStyles.classworkHead} ${selectedColumnClass(sheetColumn.id)}`} key={sheetColumn.id}><div><button aria-label={`Select ${item.title} column`} aria-pressed={selected} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", sheetColumn.id)} type="button"><strong>{item.kind === "homework" ? "HW" : "CW"}{item.position}</strong><span>{item.title}</span><small>{item.activityDate ? shortDate(item.activityDate) : "No date"}</small></button><div className={gradebookStyles.headerActions}>{copyColumnButton(sheetColumn)}<button aria-label={`Edit ${item.title} column`} onClick={() => setWorkColumnDialog(item)} type="button">Edit</button></div></div></th>;
    }
    if (sheetColumn.type === "assignment") {
    ...
    return <th className={`${gradebookStyles.assessmentHead} ${selectedColumnClass(sheetColumn.id)}`} key={sheetColumn.id}><div><button aria-label={`Select ${column.title} column`} aria-pressed={selected} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", sheetColumn.id)} type="button"><strong>{column.title}</strong><span>{shortDate(column.assessmentDate)} · {column.source === "manual" ? `Manual / ${column.maxScore}` : "Auto test"}</span><small>{column.average === null ? "No grades" : `Avg ${column.average}%`}</small></button><div className={gradebookStyles.headerActions}>{copyColumnButton(sheetColumn)}<button aria-label={`Edit ${column.title} column`} onClick={() => setGradeColumnDialog(column)} type="button">Edit</button></div></div></th>;
  }
    ...
    {embedded ? <section className={styles.sheetSection} aria-label="Topic class spreadsheet">
      ...
      <div className={`${styles.sheetScroller} ${gradebookStyles.pageScroller}`} ref={sheetScrollerRef}>
        <table className={styles.sheetTable} ref={sheetTableRef}>
          <thead><tr><th className={`${styles.sheetStudentHead} ${selectedColumnClass("students")}`} rowSpan={2}><button aria-label="Select student names column" aria-pressed={sheetSelection?.kind === "column" && sheetSelection.id === "students"} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", "students")} type="button">Student</button></th>{data.weeks.map((week) => { const expanded = expandedWeeks.has(week.label); const columns = sheetColumnsForWeek(week.label); return <th className={`${styles.sheetWeekHead} ${week.label === currentWeekLabel ? styles.sheetCurrentWeek : ""} ${expanded ? styles.sheetExpandedWeek : ""}`} colSpan={expanded ? 3 + columns.length : 1} key={week.id}><button aria-expanded={expanded} onClick={() => toggleSheetWeek(week.label)} title={`${expanded ? "Hide" : "Show"} ${week.title || week.label} columns`} type="button"><strong>{week.title || week.label}</strong><span>{week.label} · {expanded ? `${columns.length + 3} columns shown` : `${columns.length + 3} columns hidden`}</span>{week.label === currentWeekLabel ? <small>Current topic</small> : null}</button></th>; })}</tr>
          <tr>{data.weeks.map((week) => { const expanded = expandedWeeks.has(week.label); if (!expanded) { const columnId = `${week.label}:summary`; return <th className={`${styles.sheetSubhead} ${styles.starSubhead} ${selectedColumnClass(columnId)}`} key={week.id}><button aria-label={`Select ${week.label} summary column`} aria-pressed={sheetSelection?.kind === "column" && sheetSelection.id === columnId} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", columnId)} type="button">Summary</button></th>; } const summative = topicSummativeColumn(week.label); const starColumnId = `${week.label}:stars`; const skullColumnId = `${week.label}:skulls`; const finalColumnId = `${week.label}:final`; const formulaUsesN = week.finalGradeFormula ? topicGradeFormulaUsesVariable(week.finalGradeFormula, "N") : false; return <Fragment key={week.id}><th className={`${styles.sheetSubhead} ${styles.starSubhead} ${selectedColumnClass(starColumnId)}`}><button aria-label={`Select ${week.label} stars column`} aria-pressed={sheetSelection?.kind === "column" && sheetSelection.id === starColumnId} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", starColumnId)} type="button">★ Stars</button></th><th className={`${styles.sheetSubhead} ${styles.skullSubhead} ${selectedColumnClass(skullColumnId)}`}><button aria-label={`Select ${week.label} skulls column`} aria-pressed={sheetSelection?.kind === "column" && sheetSelection.id === skullColumnId} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", skullColumnId)} type="button">💀 Skulls</button></th>{sheetColumnsForWeek(week.label).map(sheetColumnHeader)}<th className={`${gradebookStyles.finalGradeHead} ${selectedColumnClass(finalColumnId)}`}><div><button aria-label={`Select ${week.label} final grade column`} aria-pressed={sheetSelection?.kind === "column" && sheetSelection.id === finalColumnId} className={styles.sheetColumnSelect} onClick={() => toggleSheetSelection("column", finalColumnId)} type="button"><strong>{week.finalGradeFormula ? `Final grade / ${week.finalGradeMax}` : "Final grade not set"}</strong><span>{week.finalGradeFormula ? summative ? `N = ${summative.title}` : formulaUsesN ? "Choose summative test" : "No summative test needed" : "No grades calculated"}</span><small>{week.finalGradeFormula ? `${week.finalGradeFormula} · clamped 0–${week.finalGradeMax}` : "Configure when the topic is complete"}</small></button><div className={gradebookStyles.headerActions}><button onClick={() => setTopicSettingsDialog(week.id)} type="button">Edit final grade</button><button className={gradebookStyles.copyColumnButton} disabled={!week.finalGradeFormula || !data.gradebookRoster.length} onClick={() => void copyFinalGradeColumn(week.label)} type="button">{copiedColumnId === `final:${week.label}` ? "✓ Copied" : "Copy grades"}</button></div></div></th></Fragment>; })}</tr></thead>
          <tbody>{sortedStudents.map((student) => <tr className={sheetSelection?.kind === "row" && sheetSelection.id === student.id ? styles.sheetSelectedRow : ""} key={student.id}><th className={`${styles.sheetStudentCell} ${selectedColumnClass("students")}`}><button aria-label={`Select ${student.fullName} row`} aria-pressed={sheetSelection?.kind === "row" && sheetSelection.id === student.id} className={styles.sheetStudentSelect} onClick={() => toggleSheetSelection("row", student.id)} type="button"><span className={styles.sheetAvatar}>{student.fullName.split(/\s+/).slice(0, 2).map((part) => part[0]).join("")}</span><span><strong>{student.fullName}</strong><small>{student.email || "Student"}</small></span></button><a aria-label={`Open ${student.fullName} profile`} className={styles.sheetStudentProfile} href={`/teacher/students/${student.id}`}>↗</a></th>{data.weeks.map((week) => {
             const topicSkulls = student.skulls[week.label] ?? { today: 0, total: 0 };
             const finalGrade = topicFinalGrade(week.label, student.id);
             if (!expandedWeeks.has(week.label)) return <td className={`${styles.sheetCollapsedCell} ${week.label === currentWeekLabel ? styles.sheetCurrentCell : ""} ${selectedColumnClass(`${week.label}:summary`)}`} key={week.id}><strong>★ {student.totals[week.label] ?? 0} · 💀 {topicSkulls.total}</strong><small>{finalGrade === null ? "Final —" : `Final ${finalGrade}/${week.finalGradeMax}`}</small></td>;
             const passing = finalGrade !== null && isPassingTopicGrade(finalGrade, week.finalGradeMax);
             return <Fragment key={week.id}><td className={`${styles.sheetActionCell} ${styles.sheetStarCell} ${selectedColumnClass(`${week.label}:stars`)}`}><strong>★ {student.totals[week.label] ?? 0}</strong><div><button aria-label={`Remove a Star from ${student.fullName} in ${week.label}`} disabled={(student.totals[week.label] ?? 0) <= 0} onClick={() => changeStars(student.id, -1, week.label)} type="button">−</button><button aria-label={`Add a Star to ${student.fullName} in ${week.label}`} className={styles.sheetStarAdd} onClick={() => changeStars(student.id, 1, week.label)} type="button">＋</button></div></td><td className={`${styles.sheetActionCell} ${styles.sheetSkullCell} ${selectedColumnClass(`${week.label}:skulls`)}`}><strong>💀 {topicSkulls.total}</strong><small>Today {topicSkulls.today}/3</small><div><button aria-label={`Clear today’s Skulls for ${student.fullName} in ${week.label}`} disabled={topicSkulls.today === 0} onClick={() => clearStudentSkullsToday(student.id, week.label)} type="button">Clear</button><button aria-label={`Add a Skull to ${student.fullName} in ${week.label}`} className={styles.sheetSkullAdd} disabled={topicSkulls.today >= 3} onClick={() => addSkull(student.id, week.label)} type="button">＋</button></div></td>{sheetColumnsForWeek(week.label).map((column) => sheetColumnCell(column, student))}<td className={`${gradebookStyles.finalGradeCell} ${finalGrade === null ? "" : passing ? gradebookStyles.finalGradePassed : gradebookStyles.finalGradeFailed} ${selectedColumnClass(`${week.label}:final`)}`}>{finalGrade === null ? <><strong title="No final grade">—</strong><small>{week.finalGradeFormula ? "Waiting for grades" : "Not configured"}</small></> : <><strong>{finalGrade} / {week.finalGradeMax}</strong><small>{passing ? "Passed · 65%+" : "Below 65%"}</small></>}</td></Fragment>;
          })}</tr>)}</tbody>
        </table>
      </div>
