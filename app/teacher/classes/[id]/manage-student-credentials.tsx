...
export default function ManageStudentCredentials({ classId, students }: { classId: string; students: Student[] }) {
  ...
  return <div className="class-credentials"><button className="secondary-inline-button" onClick={() => setOpen((current) => !current)} type="button">{open ? "Close credential manager" : "Manage credentials"}</button>{open && <div className="credential-manager"><p className="credential-warning"><strong>Warning:</strong> This will replace the selected students&apos; current passwords.</p><form action={action} onSubmit={confirmReset}><input name="class_id" type="hidden" value={classId} /><label className="credential-select-all"><input checked={allSelected} onChange={(event) => toggleAll(event.target.checked)} type="checkbox" /> Select all students</label><div className="credential-student-list">{students.map((student) => <label key={student.id}><input checked={selected.has(student.id)} name="student_id" onChange={(event) => toggleStudent(student.id, event.target.checked)} type="checkbox" value={student.id} /><span><strong>{student.fullName}</strong><small>{student.emailAddress}</small></span></label>)}</div><button className="teacher-button" disabled={pending || !selected.size} type="submit">{pending ? "Generating..." : "Generate new temporary passwords"}</button></form>{selectionError && <p className="form-error" role="alert">{selectionError}</p>}{state.error && <p className="form-error" role="alert">{state.error}</p>}{state.credentials?.length ? <div className="credential-results"><h3>New one-time credentials</h3><p>Download or print these now. Temporary passwords are not saved.</p><div className="credential-result-list">{state.credentials.map((credential) => <div key={credential.emailAddress}><strong>{credential.fullName}</strong><span>{credential.emailAddress}</span><code>{credential.temporaryPassword}</code></div>)}</div><CredentialActions credentials={state.credentials} /></div> : null}</div>}</div>;
}
