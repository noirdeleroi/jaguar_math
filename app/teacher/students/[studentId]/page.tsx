...
export default async function TeacherStudentProgressPage({ params }: PageProps) {
  ...
  return <main className="teacher-main assessment-page">
    ...
    <section className="teacher-section student-classroom-record"><div className="section-row"><div><p className="eyebrow">Teacher-only classroom record</p><h2>Stars, homework and classwork</h2></div></div><div className="student-classroom-record-grid">{classroomStarRows.map((row) => <article key={row.classId}><header><div><strong>{row.className}</strong><span>{row.total} total stars</span></div><Link href={`/teacher/classes/${row.classId}/stars`}>Open class →</Link></header><div className="student-week-stars">{row.weeks.length ? row.weeks.map((week) => <span key={week.label}><b>{week.label}</b> ⭐ {week.total}</span>) : <span>No recorded stars yet</span>}</div><p>HW: {row.statusCounts.done ?? 0} done · {row.statusCounts.late ?? 0} late · {row.statusCounts.missing ?? 0} missing</p><p>CW: {row.statusCounts.ok ?? 0} OK · {row.statusCounts.not_ok ?? 0} not OK</p></article>)}</div></section>
