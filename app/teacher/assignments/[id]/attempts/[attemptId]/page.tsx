...
export default async function TeacherAttemptDetailPage({ params }: AttemptDetailPageProps) {
  ...
  const { data: attempt, error: attemptError } = await supabase.from("attempts").select("id, student_id, attempt_number, status, score, max_score, submitted_at, exam_focus_violations, profiles(full_name, email)").eq("id", attemptId).eq("assignment_id", assignment.id).eq("status", "submitted").maybeSingle();
  ...
  const reviewQuestions: TeacherReviewQuestion[] = (composition ?? []).flatMap((item) => {
    ...
    return [{ id: item.question_id, number: item.position, prompt: question.prompt, type: question.type, options: question.options as { id: string; text: string }[] | null, studentAnswer, earnedPoints: response?.points_awarded ?? null, points: Number(item.points), isCorrect: response?.is_correct ?? null, correctAnswer: key.correct_answer, explanation: key.explanation, answered, skills: skills.map((skill) => ({ code: skill.code, name: skill.name, isPrimary: skill.is_primary })) }];
  });
  ...
  return <main className="teacher-main assessment-page teacher-attempt-page"><Link className="back-link" href={`/teacher/assignments/${assignment.id}`}>← {assignment.title}</Link><section className="page-heading teacher-attempt-heading"><div><p className="eyebrow">{autoSubmitted ? "Auto-submitted attempt" : "Submitted attempt"}</p><h1>{profile?.full_name || profile?.email || "Student"}</h1><p>Attempt {attempt.attempt_number}{attempt.submitted_at ? ` · Submitted ${new Date(attempt.submitted_at).toLocaleString()}` : ""}</p></div><Link className="teacher-attempt-student-link" href={`/teacher/students/${attempt.student_id}`}><span>Student overview</span><strong>View overall progress</strong><b aria-hidden="true">→</b></Link></section>
    ...
    {assignment.exam_mode && <details className="teacher-section teacher-attempt-activity"><summary><span><b>Exam Mode activity</b><small>{attempt.exam_focus_violations} focus {attempt.exam_focus_violations === 1 ? "exit" : "exits"} · {autoSubmitted ? "Auto-submitted" : "Submitted"}</small></span><strong aria-hidden="true">+</strong></summary><div className="exam-activity"><p>Activity signals show page/fullscreen interruptions, not proof of misconduct.</p>{examEvents?.length ? <div className="exam-activity-list">{examEvents.map((event, index) => <article key={`${event.occurred_at}-${index}`}><strong>{eventLabel[event.event_type] ?? event.event_type}</strong><span>{new Date(event.occurred_at).toLocaleString()}{event.away_duration_seconds !== null ? ` · away ${event.away_duration_seconds} sec` : ""}</span></article>)}</div> : <p className="form-note">No Exam Mode activity was recorded.</p>}</div></details>}
  </main>;
}
