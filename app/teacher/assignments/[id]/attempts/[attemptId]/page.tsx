...
export default async function TeacherAttemptDetailPage({ params }: AttemptDetailPageProps) {
  ...
  return <main className="teacher-main assessment-page teacher-attempt-page"><Link className="back-link" href={`/teacher/assignments/${assignment.id}`}>← {assignment.title}</Link><section className="page-heading"><p className="eyebrow">{autoSubmitted ? "Auto-submitted attempt" : "Submitted attempt"}</p><h1>{profile?.full_name || profile?.email || "Student"}</h1><p>Attempt {attempt.attempt_number}{attempt.submitted_at ? ` · Submitted ${new Date(attempt.submitted_at).toLocaleString()}` : ""}</p></section>
