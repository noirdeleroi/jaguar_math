...
export default function AssignmentBuilder({ classes }: { classes: Classroom[] }) {
  const [source, setSource] = useState(assignmentImportExample); const [questions, setQuestions] = useState<ImportedQuestion[] | null>(null); const [errors, setErrors] = useState<string[]>([]); const [title, setTitle] = useState(""); const [titleError, setTitleError] = useState(""); const [questionsError, setQuestionsError] = useState(""); const [formError, setFormError] = useState("");
  const [saveState, saveAction] = useActionState<DraftActionState, FormData>(createAssignment, null);
  const parse = () => { const result = parseAssignmentImport(source); setErrors(result.errors); setQuestions(result.data?.questions ?? null); setQuestionsError(result.data ? "" : "Fix the import errors, then click Validate & preview again."); };
  ...
  return <main className="teacher-main assessment-page"><Link className="back-link" href="/teacher/assignments">← Assignments</Link><section className="page-heading"><p className="eyebrow">Assessment workspace</p><h1>New assignment</h1><p>Import structured questions, review them, and save a private draft.</p><Link className="back-link" href="/teacher/questions">Reuse questions from your personal bank →</Link></section>
    <form className="assessment-form" action={saveAction} onSubmit={validateBeforeSave}>
      ...
      <section className="teacher-section"><div className="section-row"><div><h2>Question import</h2><p className="form-note">Paste the agreed ChatGPT JSON format. Math wrapped in <code>$...$</code> or <code>$$...$$</code> is rendered safely.</p></div><button className="secondary-inline-button" onClick={parse} type="button">Validate & preview</button></div><textarea aria-label="Question import JSON" className="import-textarea" onChange={(event) => { setSource(event.target.value); setQuestions(null); setQuestionsError(""); }} rows={18} value={source} />{errors.length > 0 && <ul className="import-errors" role="alert">{errors.map((error) => <li key={error}>{error}</li>)}</ul>}{questionsError && <p className="notice notice-error" role="alert">{questionsError}</p>}</section>
