...
export default function ResultsOverviewClient({ assignmentId, overview, examMode = false, allowedFocusExits = 0, live = false, tabbed = false, questionSet }: { assignmentId: string; overview: AssignmentResultsOverview; examMode?: boolean; allowedFocusExits?: number; live?: boolean; tabbed?: boolean; questionSet?: ReactNode }) {
  const router = useRouter();
  const [sort, setSort] = useState<"name" | "percentage" | "submitted_at">("name"); const [ascending, setAscending] = useState(true); const [activeTab, setActiveTab] = useState<ResultsTab>("performance"); const [now, setNow] = useState(0); const [selectedStudents, setSelectedStudents] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (!examMode || !live) return;
    const timer = window.setInterval(() => router.refresh(), 5000);
    return () => window.clearInterval(timer);
  }, [examMode, live, router]);
  ...
  const studentsPanel = <section className="results-table-section">
    <div className="section-row"><div><h3>{examMode ? "Test manager" : "Student results"}</h3><p className="form-note">{examMode ? "Live progress shows saved answers for every assigned student. Focus exits and offline recoveries are review signals, not proof of misconduct." : "Latest submitted attempt is used for scores and overview metrics."}</p></div></div>
    {testManagerToolbar}
    ...
    <div className="results-table"><table><thead><tr>{examMode && live && <th className={styles.selectionCell}>Select</th>}<th>Student</th>{examMode && <><th>Form</th><th>Progress</th><th>Time left</th></>}<th>Score</th><th>Percentage</th><th>Submitted</th><th>Time</th>{examMode && <><th>Focus exits</th><th>Recovery</th></>}<th>Status</th>{examMode && live && <th>Actions</th>}</tr></thead><tbody>{students.map((student) => {
      ...
      return <tr key={student.student_id}>
        ...
        {examMode && live && <td><div className={styles.rowActions}>{student.status === "in_progress" && student.attempt_id ? <form action={forceSubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Submit ${student.student_name}'s saved answers now?`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.submitAction} type="submit">Submit now</button></form> : student.status === "submitted" && student.attempt_id ? <form action={unsubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Unsubmit ${student.student_name}'s Test? Their answers stay saved and grading is cleared. If their timer has ended, add extra time too.`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.unsubmitAction} type="submit">Unsubmit</button></form> : <span>—</span>}</div></td>}
      </tr>;
    })}</tbody></table></div>
  ...
  if (!tabbed) return <div className="assignment-results-overview">{performancePanel}{studentsPanel}{analysisPanel}</div>;
  const tabs: { id: ResultsTab; label: string }[] = [{ id: "performance", label: "Class performance" }, { id: "students", label: examMode ? "Test manager" : "Student results" }, { id: "analysis", label: "Question analysis" }, ...(questionSet ? [{ id: "questions" as const, label: "Question set" }] : [])];
