...
export default function ResultsOverviewClient({ assignmentId, overview, examMode = false, paperMode = false, paperAnswersReleased = false, allowedFocusExits = 0, live = false, closed = false, tabbed = false, questionSet, openPaperAnswerEntry = false }: { assignmentId: string; overview: AssignmentResultsOverview; examMode?: boolean; paperMode?: boolean; paperAnswersReleased?: boolean; allowedFocusExits?: number; live?: boolean; closed?: boolean; tabbed?: boolean; questionSet?: ReactNode; openPaperAnswerEntry?: boolean }) {
  ...
  const [sort, setSort] = useState<"name" | "percentage" | "submitted_at">("name"); const [ascending, setAscending] = useState(true); const [activeTab, setActiveTab] = useState<ResultsTab>(openPaperAnswerEntry || (monitored && live) ? "students" : "performance"); const [now, setNow] = useState(0); const [selectedStudents, setSelectedStudents] = useState<string[]>([]); const [answerEntryStudentId, setAnswerEntryStudentId] = useState<string | null>(openPaperAnswerEntry ? overview.students[0]?.student_id ?? null : null);
  ...
  const studentsPanel = <section className="results-table-section" id={paperMode ? "paper-answer-manager" : undefined}>
    ...
    <div className="results-table"><table><thead><tr>{monitored && live && <th className={styles.selectionCell}>Select</th>}<th>Student</th>{monitored && <><th>Form</th><th>Progress</th><th>Time left</th></>}<th>Score</th><th>Percentage</th><th>Submitted</th><th>Time</th>{monitored && <><th>Fullscreen exits</th><th>Recovery</th></>}<th>Status</th>{monitored && (live || paperAnswerEntryAvailable) && <th>Actions</th>}</tr></thead><tbody>{students.map((student) => {
      ...
      return <tr key={student.student_id}>
        ...
        {monitored && (live || paperAnswerEntryAvailable) && <td><div className={styles.rowActions}>{paperAnswerEntryAvailable && <button className={styles.enterAnswersAction} onClick={() => setAnswerEntryStudentId(student.student_id)} type="button">Enter answers</button>}{live && student.status === "in_progress" && student.attempt_id && (!paperMode || paperAnswersReleased) ? <form action={forceSubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Submit ${student.student_name}'s saved answers now?`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.submitAction} type="submit">Submit now</button></form> : live && examMode && student.status === "submitted" && student.attempt_id ? <form action={unsubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Unsubmit ${student.student_name}'s Test? Their answers stay saved and grading is cleared. If their timer has ended, add extra time too.`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.unsubmitAction} type="submit">Unsubmit</button></form> : !paperMode && <span>—</span>}</div></td>}
      </tr>;
    })}</tbody></table></div>
