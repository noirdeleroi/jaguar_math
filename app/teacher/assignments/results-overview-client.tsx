...
import { adjustPaperAnswerTime, forceSubmitTestAttempt, grantTestExtraTime, releasePaperAnswersForStudents, unsubmitTestAttempt } from "../assignment-actions";
...
export default function ResultsOverviewClient({ assignmentId, overview, examMode = false, paperMode = false, paperAnswersReleased = false, allowedFocusExits = 0, live = false, closed = false, tabbed = false, questionSet, openPaperAnswerEntry = false }: { assignmentId: string; overview: AssignmentResultsOverview; examMode?: boolean; paperMode?: boolean; paperAnswersReleased?: boolean; allowedFocusExits?: number; live?: boolean; closed?: boolean; tabbed?: boolean; questionSet?: ReactNode; openPaperAnswerEntry?: boolean }) {
  const router = useRouter();
  const monitored = examMode || paperMode;
  const paperAnswerEntryAvailable = paperMode && (live || closed);
  const paperManagerAvailable = paperMode && (live || closed);
  const studentSelectionAvailable = (examMode && live) || paperManagerAvailable;
  const [sort, setSort] = useState<"name" | "percentage" | "submitted_at">("name"); const [ascending, setAscending] = useState(true); const [activeTab, setActiveTab] = useState<ResultsTab>(openPaperAnswerEntry || (monitored && live) || paperManagerAvailable ? "students" : "performance"); const [now, setNow] = useState(0); const [selectedStudents, setSelectedStudents] = useState<string[]>([]); const [answerEntryStudentId, setAnswerEntryStudentId] = useState<string | null>(openPaperAnswerEntry ? overview.students[0]?.student_id ?? null : null);
  ...
  const paperManagerToolbar = paperManagerAvailable ? <form action={adjustPaperAnswerTime} className={styles.managerToolbar} id="paper-answer-time-form" onSubmit={(event) => { const submitter = (event.nativeEvent as SubmitEvent).submitter as HTMLButtonElement | null; if (submitter?.value === "release") { if (!window.confirm(`Stop the waiting phase and open answer entry now for ${selectedAssignedStudents.length} selected student${selectedAssignedStudents.length === 1 ? "" : "s"}? Their individual timers will start immediately.`)) event.preventDefault(); return; } const direction = submitter?.value === "decrease" ? "remove" : "add"; if (!window.confirm(`${direction === "add" ? "Add time for" : "Remove time from"} ${selectedAssignedStudents.length} selected student${selectedAssignedStudents.length === 1 ? "" : "s"}?`)) event.preventDefault(); }}>
    <input name="assignment_id" type="hidden" value={assignmentId} />
    <div className={styles.toolbarIntro}><span className={`${styles.liveStatus} ${closed ? styles.closedStatus : ""}`}>{closed ? "Closed · teacher controls active" : paperAnswersReleased ? "Answer entry live · refreshes every 5 seconds" : "Ready before answer release"}</span><strong>Full paper test controls</strong><span>Select one student or all students, then add or remove answer time. {paperAnswersReleased ? "Open countdowns update immediately." : "The adjustment is ready when answer entry opens."}</span></div>
    <div className={styles.selectionActions}>
      <button disabled={!studentIds.length} onClick={() => setSelectedStudents(allStudentsSelected ? [] : studentIds)} type="button">{allStudentsSelected ? "Clear all" : "Select all students"}</button>
      ...
      {live && !paperAnswersReleased && <button className={styles.releaseAnswersAction} disabled={!selectedAssignedStudents.length} formAction={releasePaperAnswersForStudents} name="manager_action" type="submit" value="release">Open answers ({selectedAssignedStudents.length})</button>}
    </div>
  </form> : null;

  const studentsPanel = <section className="results-table-section" id={paperMode ? "paper-answer-manager" : undefined}>
    <div className="section-row"><div><h3>{examMode ? "Test manager" : paperMode ? "Paper test manager" : "Student results"}</h3><p className="form-note">{examMode ? "Live progress shows saved answers for every assigned student. Focus exits and offline recoveries are review signals, not proof of misconduct." : paperMode ? "Enter answers, change time, submit, or unsubmit any student from this roster—even after the assignment is closed." : "Latest submitted attempt is used for scores and overview metrics."}</p></div></div>
    ...
    <div className="results-table"><table><thead><tr>{studentSelectionAvailable && <th className={styles.selectionCell}>Select</th>}<th>Student</th>{monitored && <><th>Form</th><th>Progress</th><th>Time left</th></>}<th>Score</th><th>Percentage</th><th>Submitted</th><th>Time</th>{monitored && <><th>Fullscreen exits</th><th>Recovery</th></>}<th>Status</th>{monitored && (live || paperAnswerEntryAvailable) && <th>Actions</th>}</tr></thead><tbody>{students.map((student) => {
      const answered = Number(student.answered_count ?? 0); const total = Number(student.total_questions ?? overview.questions.length); const progress = total > 0 ? Math.min(100, Math.round(100 * answered / total)) : 0; const studentAnswersReleased = paperMode ? Boolean(student.answers_released_at || paperAnswersReleased) : true; const timeLeft = student.status === "in_progress" ? remainingTime(student.expires_at, now) : "—"; const timeEnded = timeLeft === "Time ended";
      return <tr key={student.student_id}>
        {studentSelectionAvailable && <td className={styles.selectionCell}><input aria-label={`Select ${student.student_name} for time controls`} checked={selectedStudents.includes(student.student_id)} form={paperMode ? "paper-answer-time-form" : "test-extra-time-form"} name="student_ids" onChange={() => toggleStudent(student.student_id)} type="checkbox" value={student.student_id} /></td>}
        ...
        {monitored && (live || paperAnswerEntryAvailable) && <td><div className={styles.rowActions}>
          {paperAnswerEntryAvailable && <button className={styles.enterAnswersAction} onClick={() => setAnswerEntryStudentId(student.student_id)} type="button">Enter answers</button>}
          {paperMode && live && !studentAnswersReleased && <form action={releasePaperAnswersForStudents} onSubmit={(event) => { if (!window.confirm(`Stop ${student.student_name}'s waiting phase and start their answer timer now?`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="student_ids" type="hidden" value={student.student_id} /><button className={styles.releaseAnswersAction} type="submit">Open for student</button></form>}
          {paperManagerAvailable && <div className={styles.quickTimeActions} aria-label={`Quick time controls for ${student.student_name}`}>
            <form action={adjustPaperAnswerTime}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="student_ids" type="hidden" value={student.student_id} /><input name="seconds" type="hidden" value="30" /><input name="direction" type="hidden" value="increase" /><button onClick={(event) => { if (!window.confirm(`Add 30 seconds for ${student.student_name}?`)) event.preventDefault(); }} title="Add 30 seconds" type="submit">+30s</button></form>
            <form action={adjustPaperAnswerTime}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="student_ids" type="hidden" value={student.student_id} /><input name="seconds" type="hidden" value="30" /><input name="direction" type="hidden" value="decrease" /><button onClick={(event) => { if (!window.confirm(`Remove 30 seconds from ${student.student_name}?`)) event.preventDefault(); }} title="Remove 30 seconds" type="submit">−30s</button></form>
          </div>}
          {student.status === "in_progress" && student.attempt_id && ((examMode && live) || paperManagerAvailable) ? <form action={forceSubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Submit ${student.student_name}'s saved answers now?`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.submitAction} type="submit">Submit now</button></form> : null}
          {student.status === "submitted" && student.attempt_id && ((examMode && live) || paperManagerAvailable) ? <form action={unsubmitTestAttempt} onSubmit={(event) => { if (!window.confirm(`Unsubmit ${student.student_name}'s attempt? Their answers stay saved and grading is cleared.`)) event.preventDefault(); }}><input name="assignment_id" type="hidden" value={assignmentId} /><input name="attempt_id" type="hidden" value={student.attempt_id} /><button className={styles.unsubmitAction} type="submit">Unsubmit</button></form> : null}
          {!paperMode && student.status === "not_started" && <span>—</span>}
        </div></td>}
      </tr>;
    })}</tbody></table></div>
  ...
  const tabs: { id: ResultsTab; label: string }[] = [{ id: "performance", label: "Class performance" }, { id: "students", label: examMode ? "Test manager" : paperMode ? "Paper test manager" : "Student results" }, { id: "analysis", label: "Question analysis" }, ...(questionSet ? [{ id: "questions" as const, label: "Question set" }] : [])];
