...
export default function ResultsOverviewClient({ assignmentId, overview, examMode = false, paperMode = false, paperAnswersReleased = false, allowedFocusExits = 0, live = false, closed = false, tabbed = false, questionSet }: { assignmentId: string; overview: AssignmentResultsOverview; examMode?: boolean; paperMode?: boolean; paperAnswersReleased?: boolean; allowedFocusExits?: number; live?: boolean; closed?: boolean; tabbed?: boolean; questionSet?: ReactNode }) {
  const router = useRouter();
  const monitored = examMode || paperMode;
  const paperAnswerEntryAvailable = paperMode && (paperAnswersReleased || closed);
