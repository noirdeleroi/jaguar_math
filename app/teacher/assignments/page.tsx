...
export default async function TeacherAssignmentsPage() {
  ...
  return <main className="teacher-main assessment-page"><section className="page-heading"><div><p className="eyebrow">Assessment workspace</p><h1>Assignments</h1><p>Create, review, publish, and monitor assessment work.</p></div><Link className="teacher-button" href="/teacher/assignments/new">New assignment <span>→</span></Link></section>{assignments?.length ? <section className="teacher-section assignment-list">{assignments.map((assignment) => <Link href={`/teacher/assignments/${assignment.id}`} key={assignment.id}><div><span className={`status-pill status-${assignment.status}`}>{assignment.status}</span><strong>{assignment.title}</strong><AssignmentDue dueAt={assignment.due_at} status={assignment.status} /></div><b>→</b></Link>)}</section> : <section className="empty-state"><span aria-hidden="true">∑</span><h3>No assignments yet.</h3><p>Start with a structured import and save it as a draft.</p><Link className="teacher-button" href="/teacher/assignments/new">Create an assignment <span>→</span></Link></section>}</main>;
}
