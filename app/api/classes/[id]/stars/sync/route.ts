...
function validPayload(value: unknown): value is ClassroomSyncPayload {
  ...
  return payload.weeks.every((week) => uuidPattern.test(week.id) && weekLabelPattern.test(week.label) && Number.isInteger(week.sort_order) && week.sort_order > 0)
    ...
    && payload.skullEvents.every((event) => uuidPattern.test(event.id) && uuidPattern.test(event.student_id) && weekLabelPattern.test(event.week_label) && (event.action === "add" || event.action === "clear_today") && (event.source === "classroom" || event.source === "offline_queue") && !Number.isNaN(Date.parse(event.occurred_at)))
