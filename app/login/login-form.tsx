...
import { passwordForAuthentication } from "@/lib/auth-password";
import { createClient } from "@/lib/supabase/client";

export default function LoginForm({ returnTo }: { returnTo?: string }) {
  const router = useRouter(); const [error, setError] = useState(""); const [isLoading, setIsLoading] = useState(false);
  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); const form = new FormData(event.currentTarget); const email = String(form.get("email") ?? "").trim(); const password = String(form.get("password") ?? ""); if (!email || !password) { setError("Enter both your email address and password."); return; } setIsLoading(true); const { error: signInError } = await createClient().auth.signInWithPassword({ email, password: passwordForAuthentication(password) }); if (signInError) { setError("We couldn’t sign you in with those credentials. Please try again."); setIsLoading(false); return; } const redirectPath = returnTo ? `/auth/redirect?returnTo=${encodeURIComponent(returnTo)}` : "/auth/redirect"; router.replace(redirectPath); router.refresh(); }
