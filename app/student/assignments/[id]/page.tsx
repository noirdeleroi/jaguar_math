...
export default async function StudentAssignmentPage({ params }: AssignmentPageProps) {
  ...
  if (privateSubmission) return <main className="student-page">{(assignment.kind === "test" || assignment.kind === "paper") && assignment.status === "published" && <TestAttemptRefresher />}<div className="student-container"><Link className="back-link" href="/student">← Your assignments</Link><section className="student-results"><p className="eyebrow">Submitted</p><h1>Your assessment “{assignment.title}” was submitted.</h1></section></div></main>;
