...
export default async function StudentAssignmentPage({ params }: AssignmentPageProps) {
  ...
  } else if (assignment.kind === "test" || assignment.kind === "paper") {
  ...
  const paperSessionRow = (Array.isArray(rawPaperSession) ? rawPaperSession[0] : rawPaperSession) as { server_now?: string; writing_started_at?: string | null; writing_ends_at?: string | null; answers_released_at?: string | null; answer_ends_at?: string | null; answer_duration_seconds?: number; attempt_id?: string | null; attempt_status?: string | null; form_code?: string | null; focus_violations?: number } | null;
  const paperSession: PaperSessionState | null = assignment.kind === "paper" ? {
    ...
    writingEndsAt: paperSessionRow?.writing_ends_at ?? assignment.paper_writing_ends_at, answersReleasedAt: paperSessionRow?.answers_released_at ?? assignment.questions_released_at,
  ...
  const activeRunner = activeAttempt && !isClosed && (assignment.kind === "paper" ? paperAnswersReleased : !examMode) ? <AssessmentRunner attemptId={activeAttempt.id} expiresAt={activeAttempt.expires_at} formCode={activeAttempt.form_code} paperMode={assignment.kind === "paper"} examMode={assignment.kind === "paper" && examMode ? { ...examMode, focusViolations: activeAttempt.exam_focus_violations } : undefined} questionDisplayMode={assignment.question_display_mode} questions={runnerQuestions} responsesClosed={responsesClosed} serverNow={paperSession?.serverNow} showFeedbackAfterEachQuestion={assignment.show_feedback_after_each_question} /> : null;
