...
export default async function StudentAssignmentPage({ params }: AssignmentPageProps) {
  ...
  if (privateSubmission) return <main className="student-page">{assignment.kind === "test" && assignment.status === "published" && <TestAttemptRefresher />}<div className="student-container"><Link className="back-link" href="/student">← Your assignments</Link><section className="student-results"><p className="eyebrow">Submitted</p><h1>Your assessment “{assignment.title}” was submitted.</h1></section></div></main>;
  ...
  const activeRunner = activeAttempt && !isClosed && !examMode ? <AssessmentRunner attemptId={activeAttempt.id} expiresAt={activeAttempt.expires_at} formCode={activeAttempt.form_code} questionDisplayMode={assignment.question_display_mode} questions={runnerQuestions} responsesClosed={responsesClosed} showFeedbackAfterEachQuestion={assignment.show_feedback_after_each_question} /> : null;
