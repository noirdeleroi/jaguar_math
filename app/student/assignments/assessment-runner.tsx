...
export default function AssessmentRunner({ attemptId, startedAt, durationMinutes, questions, responsesClosed = false, examMode }: { attemptId: string; startedAt: string; durationMinutes: number | null; questions: Question[]; responsesClosed?: boolean; examMode?: ExamMode }) {
  ...
  useEffect(() => {
    ...
    const fullscreen = () => {
      if (document.fullscreenElement) { void logActivity("fullscreen_restored"); return; }
      if (examMode.requireFullscreen) window.setTimeout(() => { if (document.visibilityState === "visible") void logActivity("fullscreen_exited", undefined, true); }, 250);
    };
  ...
  const restoreFullscreen = async () => {
    try { if (document.documentElement.requestFullscreen) { await document.documentElement.requestFullscreen(); await logActivity("fullscreen_restored"); } else throw new Error("unsupported"); } catch { await logActivity("fullscreen_unavailable"); setExamWarning("Fullscreen is not available in this browser. You can continue taking the assessment."); }
  };
