...
function SkillList({ skills, kind }: { skills: AssignmentSkillReviewItem[]; kind: string }) {
  ...
  return <div className="assignment-skill-list">{skills.map((skill) => {
    ...
    return <article key={skill.code}><div><strong>{skillDisplayName(skill.code)}</strong><span>{skill.mistakeCount} {skill.mistakeCount === 1 ? "mistake" : "mistakes"} in this {kind}</span></div>{video ? <Link className="assignment-skill-video" href={`/student/videos?skill=${encodeURIComponent(skill.code)}`}>Watch lesson <span>→</span></Link> : <b>Review</b>}</article>;
  })}</div>;
}

export default function AssignmentSkillReview({ skills, kind = "homework" }: { skills: AssignmentSkillReviewItem[]; kind?: string }) {
  const suggestions = skills.slice(0, 5);
  return <section className="assignment-skill-review"><header><div><p className="eyebrow">{kind === "homework" ? "Homework review" : "Released assessment review"}</p><h3>Skills to revisit</h3><p className="assignment-skill-intro">Based only on mistakes from this submitted assignment—not your overall progress. Repeated mistakes are shown first.</p></div></header><section className="assignment-skill-group practice"><h4>{skills.length > suggestions.length ? "Top skills to review" : "Skills with mistakes"}</h4><p>{skills.length > suggestions.length ? `Showing the 5 most important of ${skills.length} skills with mistakes.` : "Review these skills, then try the linked lesson."}</p><SkillList kind={kind} skills={suggestions} /></section></section>;
}
