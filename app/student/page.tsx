...
export default async function StudentPage() {
  ...
  return <StudentDashboard assignments={assignments} classes={classes} currentWeek={currentWeek} email={student.email} nickname={classroom?.nickname || student.full_name?.trim().split(/\s+/)[0] || "student"} notifications={notificationState.notifications} unreadNotificationCount={notificationState.unreadCount} satSummary={{ readiness: satProgress.readiness, assessedSkills: satProgress.assessedSkills, totalSkills: satProgress.totalSkills }} studentId={student.id} />;
}
