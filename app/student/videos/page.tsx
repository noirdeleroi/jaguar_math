...
function toVideoSkill(skill: TaxonomySkill, progressByCode: Map<string, { readiness: number | null; attempted: number; evidenceLabel: VideoSkill["evidenceLabel"] }>): VideoSkill {
  const video = videos[skill.code];
  const progress = progressByCode.get(skill.code);
  return { code: skill.code, name: skillDisplayName(skill.code), readiness: progress?.readiness ?? null, attempted: progress?.attempted ?? 0, evidenceLabel: progress?.evidenceLabel ?? "Not assessed", videoTitle: video?.youtube_video_title ?? null, videoUrl: video?.youtube_link ?? null };
}

function buildDomains(progressByCode: Map<string, { readiness: number | null; attempted: number; evidenceLabel: VideoSkill["evidenceLabel"] }>): VideoDomain[] {
  ...
  const satDomains = SAT_DOMAINS.map((domain) => ({
    ...
    topics: domain.topics.map((topic) => {
      ...
      return { name: topic.name, skills: topicSkills.map((skill) => toVideoSkill(skill, progressByCode)) };
  ...
  return [...satDomains, { code: "FOUNDATIONS", name: "Foundations and supporting skills", subtitle: "Build the skills behind SAT Math", topics: [...foundationTopics.entries()].map(([name, topicSkills]) => ({ name, skills: topicSkills.map((skill) => toVideoSkill(skill, progressByCode)) })) }];
}
...
export default async function StudentVideosPage({ searchParams }: { searchParams: Promise<{ skill?: string | string[] }> }) {
  ...
  const progress = await getStudentSatProgress(student.id);
  const progressByCode = new Map(progress.domains.flatMap((domain) => domain.topics.flatMap((topic) => topic.skills.map((skill) => [skill.code, skill] as const))));
  const domains = buildDomains(progressByCode);
