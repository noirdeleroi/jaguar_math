...
type Props = { studentId: string; nickname: string; email: string | null; classes: string[]; assignments: DashboardAssignment[]; notifications: StudentNotification[]; unreadNotificationCount: number; satSummary: SatSummary; currentWeek: CurrentWeek | null };
...
function ActiveAssignments({ assignments }: { assignments: DashboardAssignment[] }) {
  return (
    <section className="dashboard-active-assignments" aria-labelledby="active-assignments-title">
      <header>
        <div>
          <p className="eyebrow">Needs attention</p>
          <h2 id="active-assignments-title">{assignments.length === 1 ? "Active assessment" : `${assignments.length} active assessments`}</h2>
        </div>
        <Link className="dashboard-active-all" href="/student/assessments?filter=todo">View all <span aria-hidden="true">→</span></Link>
      </header>
      <div className="dashboard-active-assignment-list">
        {assignments.map((assignment) => (
          <article className="dashboard-active-assignment-card" key={assignment.id}>
            <div className="dashboard-card-main">
              <div className="dashboard-card-top">
                <span className={`dashboard-status status-${assignment.status.toLowerCase().replaceAll(" ", "-")}`}>{assignment.status}</span>
                <DueLabel assignment={assignment} compact />
              </div>
              <h3>{assignment.title}</h3>
              <p className="dashboard-class-label">{assignment.classNames.join(" · ") || "Your class"}</p>
...
export default function StudentDashboard({ studentId, nickname, email, classes, assignments, notifications, unreadNotificationCount, satSummary, currentWeek }: Props) {
  ...
  return <main className="student-page"><div className="student-container student-dashboard-container"><header className="student-header"><div className="auth-brand"><span className="brand-mark" aria-hidden="true">∑</span>Jaguar Math</div><nav className="student-header-actions" aria-label="Student menu"><Link href="/student/assessments">Assessments</Link><Link href="/student/records">Class record</Link><Link href="/student/progress">Progress</Link><Link href="/student/sat-math">SAT info</Link><Link className="student-nav-video" href="/student/videos"><span aria-hidden="true">▶</span> Study YouTube Videos</Link><NotificationCenter initialNotifications={notifications} initialUnreadCount={unreadNotificationCount} studentId={studentId} /><LogoutButton /></nav></header><section className="dashboard-welcome"><div className="dashboard-welcome-copy"><p className="eyebrow">Student space</p><h1 suppressHydrationWarning>{greeting()}, {nickname}.</h1>{classes.length ? <p>{classes.join(" · ")}</p> : <p>{email || "Your Jaguar Math workspace"}</p>}</div><DashboardReadiness summary={satSummary} /></section>{currentWeek ? <section className="dashboard-current-week" aria-label={`Current teaching week ${currentWeek.label}`}><div><span>Current teaching week</span><strong>Week {currentWeek.label}</strong></div><div><span>Grade {currentWeek.gradeLevel} topic</span><h2>{currentWeek.topic}</h2>{currentWeek.unit ? <small>{currentWeek.unit}</small> : null}<Link className="dashboard-current-week-link" href="/student/records">View your weekly Stars, HW &amp; CW <b aria-hidden="true">→</b></Link></div></section> : null}{actionable.length ? <ActiveAssignments assignments={actionable} /> : <section className="dashboard-caught-up"><p className="eyebrow">Your workspace</p><h2>You&apos;re all caught up.</h2><p>{assignments.length ? "Completed work is available on your Assessments page." : classes.length ? "Your teachers have not published any assignments yet." : "You are not enrolled in a class yet."}</p></section>}<nav className="dashboard-summary" aria-label="Assessment summary"><Link className="dashboard-summary-card" href="/student/assessments?filter=todo"><span>To do</span><strong>{toDo.length}</strong></Link><Link className="dashboard-summary-card" href="/student/assessments?filter=in-progress"><span>In progress</span><strong>{inProgress.length}</strong></Link><Link className="dashboard-summary-card" href="/student/assessments?filter=completed"><span>Completed</span><strong>{completed.length}</strong></Link></nav><SatPreparation /></div></main>;
}
