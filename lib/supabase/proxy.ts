...
export async function updateSession(request: NextRequest) {
  ...
  if (!data?.claims.sub && (path.startsWith("/student") || path.startsWith("/teacher"))) { const url = request.nextUrl.clone(); url.pathname = "/login"; url.search = ""; url.searchParams.set("returnTo", `${path}${request.nextUrl.search}`); return NextResponse.redirect(url); }
  return response;
}
