...
export async function updateSession(request: NextRequest) {
  ...
  if (!user && request.nextUrl.pathname.startsWith("/student")) { const url = request.nextUrl.clone(); url.pathname = "/login"; url.searchParams.set("next", request.nextUrl.pathname); return NextResponse.redirect(url); }
  return response;
}
