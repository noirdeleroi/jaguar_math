...
export async function getStudentAssignments(studentId: string) {
  ...
  const classNameById = new Map((classes ?? []).map((classroom) => [classroom.id, classroom.name])); const classesByAssignment = new Map<string, Set<string>>(); const questionCountByAssignment = new Map<string, number>(); const attemptsByAssignment = new Map<string, Attempt[]>(); const answeredByAttempt = new Map<string, number>();
  ...
  const now = Date.now(); const dashboardAssignments: DashboardAssignment[] = assignmentRows.map((assignment) => {
  ...
  dashboardAssignments.sort((left, right) => {
    ...
    if (left.actionable) { const priority = assignmentPriority(left) - assignmentPriority(right); if (priority) return priority; const leftDue = left.dueAt ? new Date(left.dueAt).getTime() : Number.POSITIVE_INFINITY; const rightDue = right.dueAt ? new Date(right.dueAt).getTime() : Number.POSITIVE_INFINITY; return leftDue - rightDue; }
    if (left.completed !== right.completed) return left.completed ? -1 : 1;
    return new Date(right.submittedAt ?? 0).getTime() - new Date(left.submittedAt ?? 0).getTime();
  });
  return { assignments: dashboardAssignments, classes: [...classNameById.values()] };
}
